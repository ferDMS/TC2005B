...
CREATE PROCEDURE `books`.`book_getAllBooks`()
BEGIN
    SELECT * FROM book;
END//

CREATE PROCEDURE `books`.`book_getBookByID`(IN bookID INT)
BEGIN
    SELECT * FROM book WHERE id_book = bookID;
END//

CREATE PROCEDURE `books`.`book_updateBook`(IN bookID INT, IN newTitle VARCHAR(50), IN newAuthor VARCHAR(100), IN newUrl VARCHAR(100))
BEGIN
    UPDATE book SET title = newTitle, author = newAuthor, url = newUrl WHERE id_book = bookID;
END//

CREATE PROCEDURE `books`.`book_getPagesByBookID`(IN bookID INT)
BEGIN
    SELECT * FROM pages WHERE id_book = bookID;
END//

CREATE PROCEDURE `books`.`book_insertPage`(IN bookID INT, IN pageContent VARCHAR(4000))
BEGIN
...
INSERT INTO book (title, author, url) VALUES 
